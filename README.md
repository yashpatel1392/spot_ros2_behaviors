...
The capabilities implemented in this repository include navigation, manipulation, and docking. The navigation capabilities involve uploading a graph to the Spot, localizing it to a waypoint, and autonomously navigating to a specified goal. The manipulation capabilities include moving the robot's arm to predefined positions, such as stow and unstow, controlling the arm based on joint values, and autonomously grasping objects from a given frame. Additionally, the repository supports capturing images from any camera source on the Spot, controlling the gripper, and performing docking and undocking operations.
